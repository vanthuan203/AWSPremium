...
public interface VideoViewRepository extends JpaRepository<VideoView,Long> {
    ...
    @Query(value = "SELECT * FROM videoview where service in(select service from service where checktime=0  and live=0) and timestart>0 and INSTR(?1,videoid)=0 and orderid in (?2) order by rand() limit 1",nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM videoview where service in(select service from service where checktime=0  and live=1) and timestart>0  and orderid in (?1) order by rand() limit 1",nativeQuery = true)
