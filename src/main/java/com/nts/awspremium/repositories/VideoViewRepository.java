...
public interface VideoViewRepository extends JpaRepository<VideoView,Long> {
    ...
    @Query(value = "SELECT * FROM videoview where service in(select service from service where geo='ext') and timestart>0 order by rand() limit 1",nativeQuery = true)
    public List<VideoView> getvideoViewEXT();
    ...
    @Query(value = "SELECT * FROM videoview where service in(select service from service where geo=?1 and checktime=0  and live=0 and type='Default') and timestart>0 and INSTR(?2,videoid)=0 and orderid in (?3) order by rand() limit 1",nativeQuery = true)
    public List<VideoView> getvideoViewByGeoBAS28(String geo, String listvideo, List<String> orderid);
    ...
    @Query(value = "select orderid from (select videoview.orderid,count(running) as total,maxthreads,valid,viewtotal,vieworder,speedup,threadset\n" +
            "                              from videoview left join historyview on historyview.orderid=videoview.orderid and running=1\n" +
            "                                  group by orderid having total<maxthreads or  (((select bonus/100 from setting where id=1)+1)*vieworder-viewtotal>total*0.5 and threadset*3>total and speedup=1) ) as t",nativeQuery = true)
    ...
    @Query(value = "select orderid from (select videoview.orderid,count(running) as total,maxthreads,valid,viewtotal,vieworder,speedup,threadset,timestart\n" +
            "                                        from videoview left join historyview on historyview.orderid=videoview.orderid and running=1 \n" +
            "                                            group by orderid having (((select bonus/100 from setting where id=1)+1)*vieworder-viewtotal>total*0.5  and threadset*3>total and round((UNIX_TIMESTAMP()-timestart/1000)/60)>=120 ) ) as t",nativeQuery = true)
    ...
    @Query(value = "SELECT sum(threadset) from videoview where timestart>0 and service in (select service from service where live=0)",nativeQuery = true)
    ...
    public List<VideoView> getAllOrderDoneBuffH2h30();
    ...
    @Query(value = "Select videoview.orderid,videoview.videoid,videoview.videotitle,count(running) as total,maxthreads,insertdate,timestart,videoview.note,duration,viewstart,vieworder,user,viewtotal,timeupdate,view24h,price,videoview.service,service.geo,service.ai,service.live from videoview left join historyview on historyview.videoid=videoview.videoid and running=1 left join service on videoview.service=service.service where user!='baohanh01@gmail.com' and timestart!=0 group by videoid order by timestart desc",nativeQuery = true)
    ...
    @Query(value = "Select videoview.orderid,videoview.videoid,videoview.videotitle,count(running) as total,maxthreads,insertdate,timestart,videoview.note,duration,viewstart,vieworder,user,viewtotal,timeupdate,view24h,price,videoview.service,service.geo,service.ai,service.live from videoview left join historyview on historyview.videoid=videoview.videoid and running=1 left join service on videoview.service=service.service where user=?1 and timestart!=0 group by videoid order by timestart desc",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE videoview set viewstart=?1,maxthreads=threadset,timestart=?2 where timestart=0 and maxthreads=-2 and videoid=?3",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "update videoview set valid=0 where round((UNIX_TIMESTAMP()-timeupdate/1000)/60)>30",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "update videoview set maxthreads=if(maxthreads+cast(threadset*(1/minstart) as UNSIGNED)>threadset,threadset,maxthreads+cast(threadset*(1/minstart) as UNSIGNED)) where service in (select service from service where task='view') and maxthreads<threadset and maxthreads>0 and timestart>0;",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "update videoview set maxthreads=if(maxthreads+cast(threadset*0.16 as UNSIGNED)>threadset,threadset,maxthreads+cast(threadset*0.4 as UNSIGNED)) where service in (select service from service where maxtime=4) and maxthreads<threadset and maxthreads>0 and timestart>0;",nativeQuery = true)
    ...
    @Query(value = "select * from videoview where viewtotal>vieworder*1.05 and service in(select service from service where live=1) and round((UNIX_TIMESTAMP()-timestart/1000)/60)>=(minstart+10)",nativeQuery = true)
