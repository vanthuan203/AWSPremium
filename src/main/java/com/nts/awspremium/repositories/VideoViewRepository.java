...
public interface VideoViewRepository extends JpaRepository<VideoView,Long> {
    ...
    @Query(value = "SELECT * FROM videoview where service in(select service from service where geo!=?1 and geo!='test1' and checktime=0 and live=0) and INSTR(?2,videoid)=0 and orderid in (?3) order by rand() limit 1",nativeQuery = true)
    ...
    @Query(value = "select orderid from (select videoview.orderid,count(running) as total,maxthreads,valid,viewtotal,vieworder,speedup,threadset,timestart\n" +
            "                                        from videoview left join historyview on historyview.orderid=videoview.orderid and running=1 \n" +
            "                                            group by orderid having (((select bonus/100 from setting where id=1)+1)*vieworder-viewtotal>total*2  and threadset*3>total and round((UNIX_TIMESTAMP()-timestart/1000)/60)>=60 ) ) as t",nativeQuery = true)
