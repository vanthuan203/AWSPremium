...
public interface VideoViewRepository extends JpaRepository<VideoView,Long> {
    ...
    @Query(value = "Select videoview.orderid,videoview.videoid,videoview.videotitle,count(running) as total,maxthreads,insertdate,timestart,videoview.note,duration,viewstart,vieworder,user,viewtotal,timeupdate,view24h,price,videoview.service,service.geo,service.ai from videoview left join historyview on historyview.videoid=videoview.videoid and running=1 left join service on videoview.service=service.service where user=?1 and timestart!=0 group by videoid order by timestart desc",nativeQuery = true)
