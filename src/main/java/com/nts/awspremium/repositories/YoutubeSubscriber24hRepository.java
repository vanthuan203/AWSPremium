...
public interface YoutubeSubscriber24hRepository extends JpaRepository<YoutubeSubscriber24h,String> {
    ...
    @Query(value = "select count(*) from youtube_subscriber_24h where order_key=?1 and  update_time >= (UNIX_TIMESTAMP() - ?2*60*60) * 1000",nativeQuery = true)
    public Integer count_Subscribe_OrderKey_And_Time(String username,Integer hour);
