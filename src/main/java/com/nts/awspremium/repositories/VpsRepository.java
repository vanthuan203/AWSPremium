...
public interface VpsRepository extends JpaRepository<Vps,Integer> {
    @Query(value = "select * from vps where !(vpsoption='Sub_Pending' or vpsoption='Tiktok_Pending'  or vpsoption='sub' or vpsoption='like' or vpsoption='like_sub' or vpsoption='tiktok') order by CAST(SUBSTRING_INDEX(vps, '-', -1) AS UNSIGNED) ASC",nativeQuery = true)
    ...
    @Query(value = "select * from vps where (vpsoption='TikTok_Pending' or vpsoption='tiktok') order by CAST(SUBSTRING_INDEX(vps, '-', -1) AS UNSIGNED) ASC",nativeQuery = true)
    public List<Vps> getListVPSTikTok();
    ...
    public Integer countAccountVps28(String vps);

    @Query(value = "Select count(*) from vps where vps=?1 and ((select count(*) from account where geo=?2 and running=1 and vps=?1))<(select max_acc_smm from setting where id=1)",nativeQuery = true)
    public Integer checkGetAccountSMMByThreadVps(String vps,String geo);
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE vps set vpsreset=?1,get_account=1,cmt=1,proxy=1,timeresettool=?2,dayreset=DATE_FORMAT(ADDDATE( UTC_TIMESTAMP(), INTERVAL +7 HOUR), '%d') where timeresettool=1 order by rand() limit ?3",nativeQuery = true)
    public void resetBasDailyByCron(Integer vpsreset,Long timeresettool,Integer limit);
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE vps set vpsreset=2,timeresettool=?1 where vps=?2",nativeQuery = true)
