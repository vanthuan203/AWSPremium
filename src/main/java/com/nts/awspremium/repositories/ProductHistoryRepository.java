...
import com.nts.awspremium.model.ProductHistory;
...
public interface ProductHistoryRepository extends JpaRepository<ProductHistory,Long> {
    ...
    @Query(value = "SELECT * from product_history where order_id=?1",nativeQuery = true)
    public ProductHistory getProductHistoriesById(Long order_id);
    ...
    @Query(value = "SELECT * from product_history where order_id in (?1)",nativeQuery = true)
    public List<ProductHistory> getChannelTikTokHisByListId(List<String> list_orderid);
