...
public interface ProxyRepository extends JpaRepository<Proxy, Integer> {
    ...
    @Query(value = "select * from proxy where geo='traffic' order by running asc,rand() limit 1",nativeQuery = true)
    public List<Proxy> getProxyFixAccountTraffic();
    ...
    @Query(value = "SELECT proxy FROM AccPremium.proxy where geo='test' and ipv4 in(select ipv4 from ipv4 where state=1);",nativeQuery = true)
    public List<String> getProxyVULTRTrue();

    @Query(value = "SELECT proxy FROM AccPremium.proxy where ipv4 in(select ipv4 from ipv4 where state=1);",nativeQuery = true)
    public List<String> getProxyALLRTrue();
    ...
    @Query(value = "SELECT CASE WHEN proxy_ha IS NULL OR LENGTH(proxy_ha) = 0 THEN NULL ELSE proxy_ha END AS proxy_ha " +
            "FROM proxy WHERE proxy = ?1", nativeQuery = true)
    String get_Proxy_HA(String proxy);
    ...
    @Query(value = "SELECT proxy FROM AccPremium.proxy where ipv4=?1 limit 1;",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE proxy set running=running+1,vps=?1,timeget=?2  where id=?3",nativeQuery = true)
    public Integer updateProxyGetTraffic(String vps,Long timeget,Integer id);
