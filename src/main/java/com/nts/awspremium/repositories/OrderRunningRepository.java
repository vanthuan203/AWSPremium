...
public interface OrderRunningRepository extends JpaRepository<OrderRunning,Long> {
    @Query(value = "SELECT * FROM order_running where service_id in(select service_id from service_smm where platform=?1 and task=?2 and mode=?3) and INSTR(?4,CONCAT(order_key,'|'))=0 and order_id in (?5) order by rand() limit 1",nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM order_running where service_id in(select service_id from service_smm where platform=?1 and task=?2 and mode=?3) and INSTR(?4,CONCAT(order_key,'|'))=0 and order_id in (?5) and priority>0 order by rand() limit 1",nativeQuery = true)
    ...
    @Query(value = "select order_id from (select order_running.order_id,count(running) as total,thread\n" +
            "                            from order_running left join historyview on historyview.orderid=order_running.order_id and running=1\n" +
    ...
    public Integer get_Count_By_Order_Key(String order_id);

    @Query(value = "SELECT * from order_running where order_key=?1 limit 1",nativeQuery = true)
    public OrderRunning get_Order_By_Order_Key(String order_id);
    ...
    @Query(value = "SELECT count(*) from order_running where order_key=?1 and username='refill@gmail.com' and service_id in(select service_id from service_smm where task=?2)",nativeQuery = true)
    public Integer get_Order_Refill_By_Order_Key_And_Task(String order_id,String task);

    @Query(value = "SELECT * from order_running where order_key=?1 and  service_id in(select service_id from service_smm where task=?2 and platform=?3) limit 1",nativeQuery = true)
    public OrderRunning find_Order_By_Order_Key(String order_key,String task,String platform);

    @Query(value = "SELECT o.*,s.task,s.platform from order_running o left join service_smm s on o.service_id=s.service_id where o.start_count_time=0 and s.platform=?1 and o.check_count=0 order by rand() limit 1",nativeQuery = true)
    public OrderRunningShow find_Order_By_Start_Count0(String platform);

    @Query(value = "SELECT o.*,s.task,s.platform from order_running o left join service_smm s on o.service_id=s.service_id where o.total>0 and round((UNIX_TIMESTAMP()-o.update_current_time/1000)/60)>=3 and s.platform=?1 and o.check_count=0 order by rand() limit 1",nativeQuery = true)
    ...
    @Query(value = "SELECT count(*) FROM order_running where service_id=?1 and start_time>0;",nativeQuery = true)
    ...
    @Query(value = "SELECT count(*) from order_running where order_key=?1 and service_id in(select service_id from service_smm where history=0)",nativeQuery = true)
