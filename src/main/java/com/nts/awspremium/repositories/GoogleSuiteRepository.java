...
public interface GoogleSuiteRepository extends JpaRepository<GoogleSuite,Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "update google_suite set state=1,update_time=0 where  state=0 and status=1 and round((UNIX_TIMESTAMP()-update_time/1000)/60/60)>=6;",nativeQuery = true)
    ...
    @Query(value = "SELECT GROUP_CONCAT(google_suite) FROM (SELECT google_suite  FROM AccPremium.account where geo='reg' and live=1 and status=0 group by google_suite) as t",nativeQuery = true)
