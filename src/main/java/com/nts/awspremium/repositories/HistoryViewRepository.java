...
public interface HistoryViewRepository extends JpaRepository<HistoryView,Long> {
    ...
    @Query(value = "SELECT * FROM historyview where username=?1 limit 1",nativeQuery = true)
    public HistoryView getHistoryViewByUsername(String username);

    @Query(value = "SELECT id FROM AccPremium.historyview where running=0 and (status=1 or state=0) and vps=?1 order by timeget asc limit 1;",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "update historyview set status=1 where status=0 and username in (SELECT username FROM AccPremium.account where running=1 and google_suite in (select id from google_suite where state=1)) order by rand() limit 500",nativeQuery = true)
