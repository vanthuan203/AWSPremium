...
public interface HistoryCommentRepository extends JpaRepository<HistoryComment,Long> {
    ...
    @Query(value = "SELECT id FROM AccPremium.historycomment where running=0 and status=1 and vps=?1 and (UNIX_TIMESTAMP()-task_time/1000)>0 order by timeget asc,rand() limit 1;",nativeQuery = true)
    public Long getAccToCmtNoCheckProxy(String vps);

    @Query(value = "SELECT id FROM AccPremium.historycomment where running=0 and vps=?1 and geo=?2 and status=1 and (UNIX_TIMESTAMP()-task_time/1000)>0 order by timeget asc,rand() limit 1;",nativeQuery = true)
