...
public interface VideoCommentHistoryRepository extends JpaRepository<VideoCommentHistory,Long> {
    ...
    @Query(value = "SELECT orderid,cancel,enddate,insertdate,videocommenthistory.note,price,videocommenthistory.service,user,videoid,commenttotal,commentend,commentorder,commentstart,geo,ai,lc_code,live from videocommenthistory left join service on service.service=videocommenthistory.service where round((UNIX_TIMESTAMP()-enddate/1000)/60/60/24)<=10 order by enddate desc\n",nativeQuery = true)
    public List<OrderCommentHistory> getVideoViewHistories();

    @Query(value = "SELECT orderid,cancel,enddate,insertdate,videocommenthistory.note,price,videocommenthistory.service,user,videoid,commenttotal,commentend,commentorder,commentstart,geo,ai,lc_code,live from videocommenthistory left join service on service.service=videocommenthistory.service where user=?1 and round((UNIX_TIMESTAMP()-enddate/1000)/60/60/24)<=10 order by enddate desc\n",nativeQuery = true)
