...
public interface CookieRepository extends JpaRepository<Cookie,Long> {
    @Modifying
    @Transactional
    @Query(value = "UPDATE cookie SET cookie=?1 where id=?2",nativeQuery = true)
    public void updateCookieSub(String cookie,Long id);
    @Modifying
    @Transactional
    @Query(value = "UPDATE cookie SET cookie=?1 where username=?2",nativeQuery = true)
    public void updateCookieByUsername(String cookie,String username);
    @Modifying
    @Transactional
    ...
    @Transactional
    @Query(value = "INSERT INTO cookie(username,cookie) VALUES(?1,?2)",nativeQuery = true)
    public void insertCookie(String username,String cookie);

    @Query(value = "SELECT cookie FROM cookie WHERE username=?1 limit 1",nativeQuery = true)
    public String getCookieByUsername(String username);
    ...
    @Query(value = "SELECT count(*) FROM cookie WHERE username=?1 limit 1",nativeQuery = true)
    public Integer checkCookieByUsername(String username);
