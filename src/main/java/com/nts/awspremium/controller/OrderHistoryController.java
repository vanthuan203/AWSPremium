...
public class OrderHistoryController {
    ...
    @Autowired
    private GoogleAPIKeyRepository googleKeyRepository;
    @Autowired
    ...
    @Autowired
    private ApiSMMController apiController;

    String get_key(){
        try{
            GoogleAPIKey key = googleKeyRepository.get_Google_Key();
    ...
    ResponseEntity<String> get_Order_History(@RequestHeader(defaultValue = "") String Authorization, @RequestParam(defaultValue = "") String user) {
        ...
        Admin users=userRepository.find_User_By_Token(Authorization.trim());
        ...
        try {
            ...
            for(int i=0;i<orders.length;i++){
                ...
                if(orderHistory.getService().getRefund()==0){
                    ...
                    orderHistoryRepository.save(orderHistory);
                    ///////////////////////
                    Float balance_update=userRepository.updateBalanceFine(charge_Refund,orderHistory.getUser().getUsername().trim());
    ...
    ResponseEntity<String> checkCount() {
        ...
        try {
            ...
            GoogleAPIKey key = googleKeyRepository.get_Google_Key();
            request1 = new Request.Builder().url("https://www.googleapis.com/youtube/v3/videos?key=" + key.getKey().trim() + "&fields=items(id,statistics(viewCount))&part=statistics&id=" + list_Order_Key).get().build();
            key.setCount(key.getCount() + 1);
            googleKeyRepository.save(key);
    ...
    ResponseEntity<String> refill(@RequestHeader(defaultValue = "") String Authorization,
        ...
        try {
            ...
            for(int i=0;i<orders.length;i++){
                ...
                if(orderHistory.getService().getRefund()==0){
                    status="✘ DV không bảo hành";
                }else if(orderHistory.getService().getRefill()==0){
                    ...
                    if(current_Count>=0 && current_Count>=orderHistory.getStart_count()){
                        ...
                        if(quantity_Refund<=0){
                            ...
                            quantity_Refund=quantity_Refund<5?5:quantity_Refund;
